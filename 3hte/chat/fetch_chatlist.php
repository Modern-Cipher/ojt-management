...
if (!$hte_id) {
    echo json_encode(["status" => "error", "message" => "No associated HTE found for trainer"]);
    exit();
}

// Fetch chatlist with latest message timestamp, time_in, time_out, filtered by students, coordinators, and admins
$sql = "
    SELECT 
        u.users_id,
    ...
    LEFT JOIN hte h 
        ON (h.hte_id = u.hte_id OR h.coordinator_id = u.users_id OR h.trainer_id = u.users_id)
    WHERE u.institute = ?
      AND (
          (u.role IN ('student', 'coordinator') AND h.hte_id = ?)
          OR u.role = 'admin'
      )
      AND u.users_id != ?
...
$stmt->bind_param("iiisis", $user_id, $user_id, $user_id, $institute, $hte_id, $user_id);
$stmt->execute();
$result = $stmt->get_result();
