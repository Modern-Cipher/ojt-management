...
// Get the latest posted announcement ID (most recent) for student
$latestSql = "SELECT a.announcements_id
              ...
              AND u.institute = ?
              AND (
                  (u.role IN ('admin', 'coordinator', 'trainer') AND a.role IN ('all', 'student'))
                  OR a.users_id = ?
              )
              ORDER BY a.created_at DESC LIMIT 1";

$latestStmt = $conn->prepare($latestSql);
$latestStmt->bind_param("si", $institute, $user_id);
$latestStmt->execute();
...
if ($result->num_rows > 0) {
    while ($announcement = $result->fetch_assoc()) {
        ...
        echo "<hr class='separator'><h5 class='comment-label'>Comment Section</h5>
              <div class='comment-section' style='max-height: 300px; overflow-y: auto; padding-right: 10px;'>";
        ...
        while ($comment = $commentResult->fetch_assoc()) {
            ...
            echo "<div class='comment-item'>
                    ...
                    <a href='announcement/delete_comment_student.php?comment_id=$commentId' onclick=\"return confirm('Are you sure you want to delete this comment?')\" style='margin-left:10px; color:red;' data-bs-toggle='tooltip' title='Remove'>
        ...
        echo "</div>
        <form class='comment-input mt-2' method='POST' action='announcement/save_comment_student.php'>
