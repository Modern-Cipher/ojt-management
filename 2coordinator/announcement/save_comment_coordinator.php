...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['announcement_id'], $_POST['comments'])) {
    ...
    if (!empty($comment)) {
        ...
        echo "<script>window.location.href = '../../2coordinator/announcement/announcement.php';</script>";

    } else {
        echo "<script>alert('Comment is required.'); window.location.href = '../../2coordinator/announcement/announcement.php';</script>";
    }
} else {
