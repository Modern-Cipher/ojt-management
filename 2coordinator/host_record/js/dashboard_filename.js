document.addEventListener("DOMContentLoaded", () => {
    ...
    function loadTrainerFiles(trainerId) {
    ...
    files.forEach((file) => {
                ...
                <div class="timeline-left d-flex flex-column align-items-center">
                    ...
                    <div class="d-flex justify-content-between align-items-center gap-2 flex-wrap w-100">
                        <div class="d-flex gap-2">
                            ...
                            <button class="btn btn-outline-dark btn-sm icon-btn edit-btn" tooltip-data="Update Status" data-id="${
                                file.uploads_id
                            }">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div class="form-check form-switch m-0" tooltip-data="See more">
                                <input class="form-check-input toggle-comments" type="checkbox" role="switch" data-id="${
                                    file.uploads_id
                                }">
                            </div>
                        </div>
                    ...
                    <!-- Comment Container -->
                    <div class="comment-container w-100 mt-2" data-id="${
                        file.uploads_id
                    }" style="display:none;">
                        <div class="comments-list" data-id="${file.uploads_id}"></div><br>
                        <form class="comment-form d-flex gap-2 justify-content-end flex-grow-1 mb-2" data-id="${
                            file.uploads_id
                        }">
        ...
        if (e.target.classList.contains("toggle-comments")) {
            ...
            `.comment-container[data-id="${uploadsId}"]`
            );
            if (e.target.checked) {
        ...
        commentForm.addEventListener("submit", (e) => {
            ...
            if (comment !== "") {
                postComment(uploadsId, comment, () => {
                    input.value = "";
                    const commentList = item.querySelector(
                        `.comments-list[data-id="${uploadsId}"]`
                    );
                    loadComments(uploadsId, commentList);
                    showToast("Comment added.", "success");
                });
            }
        }, { once: true }); // <== para kahit ma-reload, hindi madodoble
    }
    ...
    if (!uploadsId || !currentTrainerId) {
        showToast("Missing Upload ID or Trainer ID.", "warning");
        return;
    }
    fetch("update_status.php", {
        ...
        if (data.success) {
            ...
            loadTrainerFiles(currentTrainerId);
        } else {
    ...
    function loadComments(uploadsId, commentList) {
        fetch(`fetch_comments.php?uploads_id=${uploadsId}&uploadedby_id=${currentTrainerId}`)
            .then((response) => response.json())
            .then((data) => {
    ...
    fetch("insert_comment.php", {
        ...
        uploadedby_id: currentTrainerId,
        }),
    })
        .then((response) => response.json())
        .then((data) => {
